...
void VerifyAllocation(Graph& graph) {
    ...
    for (size_t i = 0; i < instructions.size(); ++i) {
        for (size_t j = i + 1; j < instructions.size(); ++j) {
            ...
            if (loc1.GetKind() == Location::REGISTER && loc2.GetKind() == Location::REGISTER && loc1.GetValue() == loc2.GetValue()) {
                ...
                if (interval1 && interval2) {
                    ...
                    EXPECT_FALSE(intersects) << "Live intervals for i" << inst1->GetId() << " and i" << inst2->GetId() << " use the same register and intersect.";
                }
            }
        }
    }
